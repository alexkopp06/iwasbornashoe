...
/**
 * Rezervace.tsx — Vinařství Rustikal
 * Booking / Experience landing page — mobile optimized
 */
...
interface EmojiIconProps {
  id: keyof typeof EMOJI_ICONS;
  accent: string;
}

interface VineLineProps {
  style?: React.CSSProperties;
}

interface WineGlass3DProps {
  accent?: string;
}
...
interface EmojiIconRendererProps {
  A: string;
  anim: boolean;
}
...
───────────────────────────────────────────────────────── */
function VineLine({ style }: VineLineProps) {
  return (
...
───────────────────────────────────────────────────────── */
function WineGlass3D({ accent = GOLD }: WineGlass3DProps) {
  ...
  return (
    ...
    >
      ...
      <button
        ...
        onMouseEnter={(e: React.MouseEvent<HTMLButtonElement>) => { e.currentTarget.style.background = `linear-gradient(135deg, rgba(${A},0.32), rgba(${A},0.18))`; e.currentTarget.style.boxShadow = `0 0 32px rgba(${A},0.22)`; }}
        onMouseLeave={(e: React.MouseEvent<HTMLButtonElement>) => { e.currentTarget.style.background = `linear-gradient(135deg, rgba(${A},0.18), rgba(${A},0.08))`; e.currentTarget.style.boxShadow = 'none'; }}
      >
...
const FEATURES: Feature[] = [
  {
    ...
    desc: 'Kamenné klenby z 19.století. 12 stupňů. Vůně dřeva a vína. Místo, kde čas stojí.',
    accent: '200,166,74',
  ...
  },
  {
    ...
    svg: (A) => (
      <svg viewBox="0 0 160 180" fill="none" style={{ width: '100%', height: 'auto', display: 'block' }}>
        ...
        {Array.from({length:24},(_,i)=>i).map(i=>(<line key={i} x1={+(80+60*Math.cos(i*Math.PI/12)).toFixed(6)} y1={+(128+42*Math.sin(i*Math.PI/12)).toFixed(6)} x2={+(80+66*Math.cos(i*Math.PI/12)).toFixed(6)} y2={+(128+46*Math.sin(i*Math.PI/12)).toFixed(6)} stroke={`rgba(${A},0.12)`} strokeWidth="0.6"/>))}
...
/* ─────────────────────────────────────────────────────────
   EMOJI ICONS
───────────────────────────────────────────────────────── */
const EMOJI_ICONS: Record<string, (props: EmojiIconRendererProps) => React.ReactNode> = {
  sklep: ({ A, anim }) => (
  ...
  zabava: ({ A, anim }) => (
    <svg viewBox="0 0 48 48" style={{ width: 48, height: 48, display: 'block', transform: anim ? 'scale(1.15)' : 'scale(1)', transition: 'transform 0.45s cubic-bezier(0.34,1.56,0.64,1)', filter: anim ? `drop-shadow(0 0 14px rgba(${A},0.70))` : `drop-shadow(0 2px 6px rgba(${A},0.20))` }}>
      {Array.from({length:12},(_,i)=>{const angle=(i*30)*Math.PI/180;const r1=anim?8:6;const r2=anim?20:16;return(<line key={i} x1={+(24+r1*Math.cos(angle)).toFixed(6)} y1={+(24+r1*Math.sin(angle)).toFixed(6)} x2={+(24+r2*Math.cos(angle)).toFixed(6)} y2={+(24+r2*Math.sin(angle)).toFixed(6)} stroke={`rgba(${A},${i%3===0?0.90:0.45})`} strokeWidth={i%3===0?1.4:0.8} strokeLinecap="round" style={{transition:'all 0.4s'}}/>);})}
      {Array.from({length:12},(_,i)=>{const angle=(i*30+15)*Math.PI/180;const r1=anim?10:8;const r2=anim?15:12;return(<line key={i} x1={+(24+r1*Math.cos(angle)).toFixed(6)} y1={+(24+r1*Math.sin(angle)).toFixed(6)} x2={+(24+r2*Math.cos(angle)).toFixed(6)} y2={+(24+r2*Math.sin(angle)).toFixed(6)} stroke={`rgba(${A},0.28)`} strokeWidth="0.6" strokeLinecap="round" style={{transition:'all 0.4s'}}/>);})}
...
function EmojiIcon({ id, accent }: EmojiIconProps) {
...
/* ─────────────────────────────────────────────────────────
   MOBILE ACCORDION STRIP
   On mobile: vertical accordion where tapping a card expands it
───────────────────────────────────────────────────────── */
function MobileStrip() {
  const [openId, setOpenId] = useState<string>(FEATURES[0].id);

  return (
    <div style={{ margin: '0 16px clamp(60px,8vh,100px)', display: 'flex', flexDirection: 'column', gap: 0, border: `1px solid rgba(${GOLD},0.12)`, overflow: 'hidden' }}>
      {FEATURES.map((f) => {
        ...
        return (
          <div
            key={f.id}
            onClick={() => { setOpenId(f.id); window.open(BOOKING_URL, '_blank', 'noopener,noreferrer'); }}
            style={{
              background: f.bg,
              borderBottom: `1px solid rgba(${A},0.10)`,
              position: 'relative',
              overflow: 'hidden',
              cursor: 'pointer',
            }}
          >
            {/* Top accent line */}
            <div style={{ position: 'absolute', top: 0, left: 0, right: 0, height: isOpen ? 2 : 1, background: `linear-gradient(90deg, transparent, rgba(${A},${isOpen ? 0.9 : 0.25}), transparent)`, transition: 'height 0.3s, background 0.3s' }}/>
            ...
            <div
              ...
              onClick={(e) => { e.stopPropagation(); setOpenId(isOpen ? '' : f.id); }}
            >
              <EmojiIcon id={f.id as keyof typeof EMOJI_ICONS} accent={f.accent}/>
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ fontFamily: 'var(--font-body,"Raleway",sans-serif)', fontSize: 8, fontWeight: 700, letterSpacing: '0.30em', textTransform: 'uppercase', color: `rgba(${A},${isOpen ? 0.80 : 0.45})`, marginBottom: 3, transition: 'color 0.3s' }}>{f.tag}</div>
                <div style={{ fontFamily: 'var(--font-display,"Cormorant Garamond",serif)', fontSize: 'clamp(20px,5vw,28px)', fontWeight: 600, color: `rgba(${CREAM},${isOpen ? 1 : 0.80})`, lineHeight: 1, letterSpacing: '0.01em', transition: 'color 0.3s' }}>{f.title}</div>
              </div>
              {/* Chevron */}
              <div style={{ flexShrink: 0, width: 24, height: 24, display: 'flex', alignItems: 'center', justifyContent: 'center', border: `1px solid rgba(${A},${isOpen ? 0.55 : 0.20})`, borderRadius: '50%', transition: 'all 0.3s', transform: isOpen ? 'rotate(180deg)' : 'rotate(0deg)' }}>
                <svg width="10" height="6" viewBox="0 0 10 6" fill="none">
                  <path d="M1 1l4 4 4-4" stroke={`rgba(${A},${isOpen ? 1 : 0.5})`} strokeWidth="1.4" strokeLinecap="round" strokeLinejoin="round"/>
                </svg>
              </div>
            </div>
            ...
            <div style={{
              ...
              <div style={{ padding: '0 20px 24px' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 16 }}>
                  <div style={{ flex: 1, height: 1, background: `rgba(${A},0.18)` }}/>
                  <div style={{ width: 4, height: 4, borderRadius: '50%', background: `rgba(${A},0.65)`, boxShadow: `0 0 6px rgba(${A},0.5)` }}/>
                  <div style={{ flex: 1, height: 1, background: `rgba(${A},0.18)` }}/>
                </div>

                {/* SVG illustration */}
                <div style={{ width: '100%', maxWidth: 180, margin: '0 auto 16px', filter: `drop-shadow(0 4px 24px rgba(${A},0.25))` }}>
                  {f.svg(A)}
                </div>

                {/* Description */}
                <p style={{ fontFamily: 'var(--font-body,"Raleway",sans-serif)', fontSize: 13, fontWeight: 400, lineHeight: 1.80, color: `rgba(${CREAM},0.62)`, margin: '0 0 18px', letterSpacing: '0.02em', textAlign: 'center' }}>{f.desc}</p>

                {/* CTA */}
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10 }}>
                  <span style={{ fontFamily: 'var(--font-body,"Raleway",sans-serif)', fontSize: 8, fontWeight: 700, letterSpacing: '0.28em', textTransform: 'uppercase', color: `rgba(${A},0.90)` }}>Rezervovat</span>
                  <div style={{ width: 24, height: 1, background: `rgba(${A},0.70)` }}/>
                </div>
              </div>
            </div>
          </div>
...
/* ─────────────────────────────────────────────────────────
   DESKTOP HORIZONTAL COMIC STRIP
───────────────────────────────────────────────────────── */
        ...
        return (
          ...
          >
            ...
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', padding: 'clamp(36px,4vh,56px) clamp(14px,1.5vw,24px) clamp(24px,3vh,40px)', height: '100%', position: 'relative', zIndex: 2, minHeight: 480 }}>
              <div style={{ marginBottom: 10 }}><EmojiIcon id={f.id as keyof typeof EMOJI_ICONS} accent={f.accent}/></div>
...
function HorizontalStrip() {
  ...
  if (isMobile || isTablet) return <MobileStrip />;
  return <DesktopStrip />;
}
...
export default function Rezervace() {
  ...
  useEffect(() => {
    setParticles(
      Array.from({ length: isMobile ? 14 : 28 }, (_, i) => ({
  ...
  const handleBook = (): void => { window.open(BOOKING_URL, '_blank', 'noopener,noreferrer'); };

  /* ── Dynamic section padding ── */
  const sectionPadH = isMobile ? '48px 20px' : isTablet ? '64px 32px' : 'clamp(80px,10vh,140px) clamp(32px,8vw,120px)';
  const sectionPad = sectionPadH;
  ...
  return (
    <div ref={pageRef} style={{ background: '#2a0202', color: `rgba(${CREAM},1)`, fontFamily: 'var(--font-body,"Raleway",sans-serif)', overflowX: 'hidden', position: 'relative' }}>
      ...
      <section ref={heroRef} style={{
        ...
        alignItems: 'center',
        padding: isMobile
          ? 'clamp(100px,18vw,130px) 20px 60px'
          : isTablet
            ? 'clamp(110px,14vh,140px) 32px 80px'
            : 'clamp(120px,14vh,160px) clamp(32px,8vw,120px) clamp(80px,10vh,140px)',
        background: 'linear-gradient(160deg, #2a0202 0%, #4a0404 50%, #2a0202 100%)',
        overflow: 'hidden',
        gap: isSmall ? '32px' : 'clamp(40px,6vw,100px)',
      }}>
        ...
        <div style={{ position: 'relative', zIndex: 10 }}>
          <div className="hero-pre" style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 24, opacity: 0 }}>
            <div style={{ width: 24, height: 1, background: `rgba(${GOLD},0.7)` }}/>
            <span style={{ fontFamily: 'var(--font-body,"Raleway",sans-serif)', fontSize: isMobile ? 7.5 : 8.5, fontWeight: 700, letterSpacing: '0.28em', textTransform: 'uppercase', color: `rgba(${GOLD},0.85)` }}>Vinařství Rustikal · Morava</span>
          </div>
          <div style={{ overflow: 'hidden', marginBottom: 6 }}>
            <h1 className="hero-title-line" style={{ fontFamily: 'var(--font-display,"Cormorant Garamond","Playfair Display",serif)', fontSize: isMobile ? 'clamp(38px,10vw,52px)' : 'clamp(42px,5.5vw,82px)', fontWeight: 300, lineHeight: 0.95, color: `rgba(${CREAM},0.92)`, margin: 0, letterSpacing: '0.02em' }}>Zažijte</h1>
          </div>
          <div style={{ overflow: 'hidden', marginBottom: 6 }}>
            <h1 className="hero-title-line" style={{ fontFamily: 'var(--font-display,"Cormorant Garamond","Playfair Display",serif)', fontSize: isMobile ? 'clamp(48px,13vw,70px)' : 'clamp(52px,7vw,110px)', fontWeight: 600, lineHeight: 0.90, margin: 0, letterSpacing: '-0.01em', background: 'linear-gradient(135deg, #fffbe0 0%, #e8c050 35%, #f5d96e 58%, #c89628 85%, #e8c050 100%)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', backgroundClip: 'text', filter: 'drop-shadow(0 2px 8px rgba(200,144,40,0.4))' }}>Rustikal</h1>
          </div>
          <div style={{ overflow: 'hidden', marginBottom: 28 }}>
            <h1 className="hero-title-line" style={{ fontFamily: 'var(--font-display,"Cormorant Garamond","Playfair Display",serif)', fontSize: isMobile ? 'clamp(20px,5.5vw,30px)' : 'clamp(26px,3.2vw,48px)', fontWeight: 300, lineHeight: 1.1, color: `rgba(${CREAM},0.65)`, margin: 0, letterSpacing: '0.12em', fontStyle: 'italic' }}>od roku 2007</h1>
          </div>

          {/* Glass — on mobile, show compact version inline */}
          {isMobile && (
            <div className="hero-glass" style={{ margin: '0 auto 28px', opacity: 0, position: 'relative', maxWidth: 180 }}>
              <WineGlass3D accent={GOLD}/>
              <div style={{ position: 'absolute', top: '8%', right: '-5%', background: `rgba(${WINE},0.88)`, border: `1px solid rgba(${GOLD},0.28)`, padding: '7px 12px', backdropFilter: 'blur(8px)', animation: 'floatCard 4s 0.5s ease-in-out infinite alternate' }}>
                <div style={{ fontFamily: 'var(--font-display,"Cormorant Garamond",serif)', fontSize: 15, fontWeight: 600, color: `rgba(${GOLD},1)` }}>38+</div>
                <div style={{ fontFamily: 'var(--font-body,"Raleway",sans-serif)', fontSize: 6.5, fontWeight: 700, letterSpacing: '0.18em', textTransform: 'uppercase', color: `rgba(${CREAM},0.55)` }}>Druhů vín</div>
              </div>
              <div style={{ position: 'absolute', bottom: '20%', left: '-10%', background: `rgba(${WINE},0.88)`, border: `1px solid rgba(${GOLD},0.28)`, padding: '7px 12px', backdropFilter: 'blur(8px)', animation: 'floatCard 3.8s 1.2s ease-in-out infinite alternate' }}>
                <div style={{ fontFamily: 'var(--font-display,"Cormorant Garamond",serif)', fontSize: 15, fontWeight: 600, color: `rgba(${GOLD},1)` }}>12°C</div>
                <div style={{ fontFamily: 'var(--font-body,"Raleway",sans-serif)', fontSize: 6.5, fontWeight: 700, letterSpacing: '0.18em', textTransform: 'uppercase', color: `rgba(${CREAM},0.55)` }}>Ve sklepě</div>
              </div>
            </div>
          )}

          <p className="hero-sub" style={{ fontFamily: 'var(--font-body,"Raleway",sans-serif)', fontSize: isMobile ? 13 : 'clamp(13px,1.1vw,16px)', fontWeight: 400, lineHeight: 1.9, color: `rgba(${CREAM},0.65)`, maxWidth: '44ch', margin: '0 0 36px', letterSpacing: '0.03em', opacity: 0 }}>
          ...
          <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap' }}>
            <a href={BOOKING_URL} target="_blank" rel="noopener noreferrer" className="hero-cta"
              style={{ display: 'inline-flex', alignItems: 'center', gap: 10, padding: isMobile ? '14px 24px' : '16px 32px', background: `linear-gradient(135deg, rgba(${GOLD},0.22), rgba(${GOLD},0.12))`, border: `1px solid rgba(${GOLD},0.65)`, fontFamily: 'var(--font-body,"Raleway",sans-serif)', fontSize: isMobile ? 8 : 9, fontWeight: 700, letterSpacing: '0.24em', textTransform: 'uppercase', color: `rgba(${GOLD},1)`, textDecoration: 'none', transition: 'all 0.28s', opacity: 0 }}
              onMouseEnter={(e: React.MouseEvent<HTMLAnchorElement>) => { e.currentTarget.style.background = `linear-gradient(135deg, rgba(${GOLD},0.36), rgba(${GOLD},0.20))`; e.currentTarget.style.transform = 'translateY(-2px)'; }}
              onMouseLeave={(e: React.MouseEvent<HTMLAnchorElement>) => { e.currentTarget.style.background = `linear-gradient(135deg, rgba(${GOLD},0.22), rgba(${GOLD},0.12))`; e.currentTarget.style.transform = 'translateY(0)'; }}
            >
            ...
            {/* ── ZMĚNA: přidán onClick pro smooth scroll na #zazitky ── */}
            <a
              href="#zazitky"
              className="hero-cta"
              onClick={(e: React.MouseEvent<HTMLAnchorElement>) => {
                e.preventDefault();
                document.getElementById('zazitky')?.scrollIntoView({ behavior: 'smooth' });
              }}
              style={{ display: 'inline-flex', alignItems: 'center', gap: 10, padding: isMobile ? '14px 20px' : '16px 28px', background: 'none', border: `1px solid rgba(${CREAM},0.18)`, fontFamily: 'var(--font-body,"Raleway",sans-serif)', fontSize: isMobile ? 8 : 9, fontWeight: 600, letterSpacing: '0.18em', textTransform: 'uppercase', color: `rgba(${CREAM},0.60)`, textDecoration: 'none', transition: 'all 0.28s', opacity: 0 }}
              onMouseEnter={(e: React.MouseEvent<HTMLAnchorElement>) => { e.currentTarget.style.color = `rgba(${CREAM},0.95)`; e.currentTarget.style.borderColor = `rgba(${CREAM},0.45)`; }}
              onMouseLeave={(e: React.MouseEvent<HTMLAnchorElement>) => { e.currentTarget.style.color = `rgba(${CREAM},0.60)`; e.currentTarget.style.borderColor = `rgba(${CREAM},0.18)`; }}
            >
              Prohlédnout zážitky
            </a>
          </div>
        ...
        {/* RIGHT — wine glass (desktop/tablet only) */}
        {!isMobile && (
        ...
        <div style={{ position: 'absolute', bottom: 28, left: '50%', transform: 'translateX(-50%)', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8, animation: 'scrollPulse 2.5s ease-in-out infinite', zIndex: 20 }}>
          <span style={{ fontFamily: 'var(--font-body,"Raleway",sans-serif)', fontSize: 7, fontWeight: 700, letterSpacing: '0.28em', textTransform: 'uppercase', color: `rgba(${CREAM},0.28)` }}>Rolujte dolů</span>
          <div style={{ width: 1, height: isMobile ? 24 : 36, background: `linear-gradient(to bottom, rgba(${GOLD},0.6), transparent)` }}/>
        </div>
      </section>
      ...
      <section id="zazitky" ref={packagesRef} style={{ background: 'linear-gradient(180deg, #2a0202 0%, #3a0404 100%)', position: 'relative', overflow: 'hidden' }}>
        <div style={{ textAlign: 'center', padding: isMobile ? '48px 20px 24px' : 'clamp(60px,8vh,100px) clamp(32px,8vw,120px) clamp(32px,4vh,52px)', position: 'relative', zIndex: 10 }}>
          <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', gap: 12, marginBottom: 16 }}>
            <div style={{ flex: 1, maxWidth: 48, height: 1, background: `rgba(${GOLD},0.30)` }}/>
            <span style={{ fontFamily: 'var(--font-body,"Raleway",sans-serif)', fontSize: isMobile ? 7 : 8, fontWeight: 700, letterSpacing: '0.36em', textTransform: 'uppercase', color: `rgba(${GOLD},0.60)` }}>Co na vás čeká</span>
            <div style={{ flex: 1, maxWidth: 48, height: 1, background: `rgba(${GOLD},0.30)` }}/>
          </div>
          <h2 style={{ fontFamily: 'var(--font-display,"Cormorant Garamond",serif)', fontSize: isMobile ? 'clamp(28px,8vw,42px)' : 'clamp(34px,4.5vw,68px)', fontWeight: 400, color: `rgba(${CREAM},0.95)`, margin: 0, letterSpacing: '0.01em' }}>
      ...
      <div ref={factsRef} style={{
        ...
        background: `rgba(${WINE},0.35)`,
        backdropFilter: 'blur(10px)',
      }}>
        {FACTS.map((f, i) => (
          <div key={f.label} className="fact-item" style={{
            opacity: 0,
            padding: isMobile ? '24px 16px' : 'clamp(28px,4vh,44px) clamp(20px,3vw,40px)',
            textAlign: 'center',
            borderRight: isMobile
              ? (i % 2 === 0 ? `1px solid rgba(${GOLD},0.10)` : 'none')
              : (i < FACTS.length - 1 ? `1px solid rgba(${GOLD},0.10)` : 'none'),
            borderBottom: isMobile && i < 2 ? `1px solid rgba(${GOLD},0.10)` : 'none',
          }}>
            <div style={{ fontFamily: 'var(--font-display,"Cormorant Garamond",serif)', fontSize: isMobile ? 'clamp(26px,7vw,38px)' : 'clamp(32px,3.5vw,52px)', fontWeight: 600, color: `rgba(${GOLD},1)`, lineHeight: 1, marginBottom: 6, letterSpacing: '-0.02em' }}>{f.num}</div>
            <div style={{ fontFamily: 'var(--font-body,"Raleway",sans-serif)', fontSize: 7.5, fontWeight: 700, letterSpacing: '0.24em', textTransform: 'uppercase', color: `rgba(${CREAM},0.45)` }}>{f.label}</div>
          </div>
        ))}
      </div>

      {/* ── ATMOSPHERE ────────────────────────────────────── */}
      <section ref={atmosphereRef} style={{
        padding: sectionPad,
        display: 'grid',
        gridTemplateColumns: isSmall ? '1fr' : '1fr 1fr',
        gap: isSmall ? '40px' : 'clamp(40px,8vw,120px)',
        alignItems: 'center',
        background: 'linear-gradient(180deg, #2a0202 0%, #3a0303 100%)',
        position: 'relative',
        overflow: 'hidden',
      }}>
        ...
        <div>
          <div className="atm-item" style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 20, opacity: 0 }}>
            <div style={{ width: 24, height: 1, background: `rgba(${GOLD},0.6)` }}/>
            <span style={{ fontFamily: 'var(--font-body,"Raleway",sans-serif)', fontSize: 8, fontWeight: 700, letterSpacing: '0.30em', textTransform: 'uppercase', color: `rgba(${GOLD},0.75)` }}>Náš příběh</span>
          </div>
          <h2 className="atm-item" style={{ fontFamily: 'var(--font-display,"Cormorant Garamond",serif)', fontSize: isMobile ? 'clamp(28px,8vw,40px)' : 'clamp(32px,4vw,60px)', fontWeight: 400, lineHeight: 1.1, color: `rgba(${CREAM},0.95)`, margin: '0 0 24px', letterSpacing: '0.01em', opacity: 0 }}>
          ...
          {[
            ...
            <p key={text} className="atm-item" style={{ fontFamily: 'var(--font-body,"Raleway",sans-serif)', fontSize: isMobile ? 13 : 'clamp(13px,1.0vw,15px)', fontWeight: 400, lineHeight: 1.92, color: `rgba(${CREAM},0.62)`, margin: '0 0 16px', letterSpacing: '0.025em', opacity: 0 }}>{text}</p>
          ))}
          <div className="atm-item" style={{ display: 'flex', alignItems: 'center', gap: 10, marginTop: 28, opacity: 0 }}>
            <a href={BOOKING_URL} target="_blank" rel="noopener noreferrer" style={{ display: 'inline-flex', alignItems: 'center', gap: 10, textDecoration: 'none', fontFamily: 'var(--font-body,"Raleway",sans-serif)', fontSize: 8.5, fontWeight: 700, letterSpacing: '0.26em', textTransform: 'uppercase', color: `rgba(${GOLD},1)`, transition: 'gap 0.24s' }}
              onMouseEnter={(e: React.MouseEvent<HTMLAnchorElement>) => { e.currentTarget.style.gap = '18px'; }}
              onMouseLeave={(e: React.MouseEvent<HTMLAnchorElement>) => { e.currentTarget.style.gap = '10px'; }}
            >
              Rezervovat místo
              <div style={{ width: 28, height: 1, background: `rgba(${GOLD},0.75)` }}/>
            </a>
          </div>
        </div>
        <div className="atm-item" style={{ opacity: 0, display: 'grid', gridTemplateColumns: isSmall ? '1fr' : '1fr 1fr', gridTemplateRows: 'auto', gap: 10 }}>
          {[
            { label: 'Historický sklípek', sub: 'Nádherné prostory z 19. století', icon: 'sklep', accent: GOLD, span: !isSmall },
            { label: 'Rodinná tradice', sub: 'Přes 19 let vášnivého pěstování', icon: 'vino', accent: '186,62,58', span: false },
            { label: 'Degustace & Events', sub: 'Intimně i velkolepě — pro 2 až 50 hostů', icon: 'zabava', accent: '148,195,128', span: !isSmall },
          ].map(tile => (
            <div key={tile.label} style={{ gridColumn: tile.span ? '1 / -1' : 'auto', padding: 'clamp(16px,2vw,24px)', background: `linear-gradient(135deg, rgba(${WINE},0.7), rgba(30,2,2,0.8))`, border: `1px solid rgba(${GOLD},0.14)`, position: 'relative', overflow: 'hidden', transition: 'border-color 0.3s', display: 'flex', alignItems: 'center', gap: 14 }}
              onMouseEnter={(e: React.MouseEvent<HTMLDivElement>) => { e.currentTarget.style.borderColor = `rgba(${GOLD},0.35)`; }}
              onMouseLeave={(e: React.MouseEvent<HTMLDivElement>) => { e.currentTarget.style.borderColor = `rgba(${GOLD},0.14)`; }}
            >
              <div style={{ flexShrink: 0 }}><EmojiIcon id={tile.icon as keyof typeof EMOJI_ICONS} accent={tile.accent}/></div>
              <div>
                <div style={{ fontFamily: 'var(--font-display,"Cormorant Garamond",serif)', fontSize: isMobile ? 16 : 'clamp(15px,1.5vw,20px)', fontWeight: 600, color: `rgba(${CREAM},0.92)`, marginBottom: 3 }}>{tile.label}</div>
                <div style={{ fontFamily: 'var(--font-body,"Raleway",sans-serif)', fontSize: 9.5, fontWeight: 500, color: `rgba(${CREAM},0.42)`, letterSpacing: '0.03em' }}>{tile.sub}</div>
              </div>
              <div style={{ position: 'absolute', bottom: 0, right: 0, width: 32, height: 32, borderTop: `1px solid rgba(${GOLD},0.18)`, borderLeft: `1px solid rgba(${GOLD},0.18)`, transform: 'translate(50%,50%) rotate(45deg)', pointerEvents: 'none' }}/>
            </div>
          ))}
        </div>
      </section>
      ...
      {/* ── FINAL CTA ─────────────────────────────────────── */}
      <section ref={finalCtaRef} style={{ position: 'relative', padding: isMobile ? '80px 20px' : 'clamp(100px,14vh,180px) clamp(32px,8vw,120px)', textAlign: 'center', overflow: 'hidden', background: 'linear-gradient(180deg, #2a0202 0%, #4a0404 50%, #2a0202 100%)' }}>
        ...
        <div aria-hidden style={{ position: 'absolute', top: 0, left: '50%', transform: 'translateX(-50%)', width: 240, height: 1, background: `linear-gradient(90deg, transparent, rgba(${GOLD},0.45), transparent)` }}/>
        <div className="final-reveal" style={{ opacity: 0 }}><VineLine style={{ display: 'block', width: 140, height: 22, margin: '0 auto 28px' }}/></div>
        <h2 className="final-reveal" style={{ fontFamily: 'var(--font-display,"Cormorant Garamond",serif)', fontSize: isMobile ? 'clamp(32px,9vw,52px)' : 'clamp(36px,5.5vw,84px)', fontWeight: 300, lineHeight: 1.05, color: `rgba(${CREAM},0.95)`, margin: '0 0 12px', letterSpacing: '0.02em', opacity: 0 }}>Vaše místo</h2>
        <h2 className="final-reveal" style={{ fontFamily: 'var(--font-display,"Cormorant Garamond",serif)', fontSize: isMobile ? 'clamp(38px,10vw,60px)' : 'clamp(44px,6.5vw,100px)', fontWeight: 600, lineHeight: 1.0, margin: '0 0 28px', letterSpacing: '-0.01em', background: 'linear-gradient(135deg, #fffbe0 0%, #e8c050 35%, #f5d96e 58%, #c89628 85%)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', backgroundClip: 'text', opacity: 0 }}>čeká ve sklepě</h2>
        <p className="final-reveal" style={{ fontFamily: 'var(--font-body,"Raleway",sans-serif)', fontSize: isMobile ? 13 : 'clamp(13px,1.1vw,16px)', fontWeight: 400, color: `rgba(${CREAM},0.55)`, lineHeight: 1.9, maxWidth: '48ch', margin: '0 auto 44px', letterSpacing: '0.025em', opacity: 0 }}>
        ...
        <a href={BOOKING_URL} target="_blank" rel="noopener noreferrer" className="final-reveal"
          style={{ display: 'inline-flex', alignItems: 'center', gap: isMobile ? 12 : 16, padding: isMobile ? '16px 28px' : '20px 48px', background: `linear-gradient(135deg, rgba(${GOLD},0.25) 0%, rgba(${GOLD},0.10) 100%)`, border: `1px solid rgba(${GOLD},0.65)`, fontFamily: 'var(--font-body,"Raleway",sans-serif)', fontSize: isMobile ? 8.5 : 10, fontWeight: 700, letterSpacing: isMobile ? '0.22em' : '0.30em', textTransform: 'uppercase', color: `rgba(${GOLD},1)`, textDecoration: 'none', transition: 'all 0.32s cubic-bezier(0.16,1,0.3,1)', opacity: 0 }}
          onMouseEnter={(e: React.MouseEvent<HTMLAnchorElement>) => { e.currentTarget.style.background = `linear-gradient(135deg, rgba(${GOLD},0.40) 0%, rgba(${GOLD},0.22) 100%)`; e.currentTarget.style.transform = 'translateY(-3px)'; }}
          onMouseLeave={(e: React.MouseEvent<HTMLAnchorElement>) => { e.currentTarget.style.background = `linear-gradient(135deg, rgba(${GOLD},0.25) 0%, rgba(${GOLD},0.10) 100%)`; e.currentTarget.style.transform = 'translateY(0)'; }}
        >
          <svg width="16" height="16" viewBox="0 0 18 18" fill="none"><rect x="1" y="3" width="16" height="14" rx="1.5" stroke={`rgba(${GOLD},0.9)`} strokeWidth="1.2"/><path d="M1 7h16" stroke={`rgba(${GOLD},0.9)`} strokeWidth="1.2"/><path d="M5 1v4M13 1v4" stroke={`rgba(${GOLD},0.9)`} strokeWidth="1.2" strokeLinecap="round"/></svg>
          Otevřít rezervační systém
        </a>
        <div aria-hidden style={{ position: 'absolute', bottom: 0, left: '50%', transform: 'translateX(-50%)', width: 240, height: 1, background: `linear-gradient(90deg, transparent, rgba(${GOLD},0.25), transparent)` }}/>
      </section>

      {/* ── FOOTER ───────────────────────────────────────── */}
      <div style={{ display: 'flex', flexDirection: isMobile ? 'column' : 'row', justifyContent: 'space-between', alignItems: 'center', padding: isMobile ? '20px 20px' : '20px clamp(32px,8vw,120px)', borderTop: `1px solid rgba(${GOLD},0.08)`, background: '#200101', gap: 8, textAlign: isMobile ? 'center' : 'left' }}>
        ...
        <span style={{ fontFamily: 'var(--font-body,"Raleway",sans-serif)', fontSize: 8.5, fontWeight: 500, letterSpacing: '0.12em', color: `rgba(${CREAM},0.28)` }}>Hustopečská podoblast · Morava · Česká republika</span>
      </div>
      ...
      <style>{`
        ...
        /* ── Prevent horizontal scroll from decorative circles ── */
