...
const WINE_CATALOGUE: WineProduct[] = [
...
/* ── Tiny bottle icon ───────────────────────────────────────── */
function BottleIcon({ color, active }: BottleIconProps) {
  return (
...
/* ── Quantity stepper ───────────────────────────────────────── */
function Stepper({ value, onChange, max = 24 }: StepperProps) {
...
/* ── Wine selector card ─────────────────────────────────────── */
function WineCard({ wine, qty, onChange }: WineCardProps) {
...
function FloatInput({ id, label, type = 'text', value, onChange, required, autoComplete, span }: FloatInputProps) {
...
function SummaryItem({ wine, qty, onRemove }: SummaryItemProps) {
...
export default function OrderForm() {
  ...
  /* Customer fields */
  const [fields, setFields] = useState<CustomerFields>({ name: '', email: '', phone: '', address: '', note: '' });

  /* Submission state */
  const [submitted, setSubmitted]   = useState<boolean>(false);
  const [submitting, setSubmitting] = useState<boolean>(false);

  const setQty   = useCallback((id: string, qty: number) => setQuantities(q => ({ ...q, [id]: qty })), []);
  const setField = useCallback((key: keyof CustomerFields) => (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => setFields(f => ({ ...f, [key]: e.target.value })), []);
  ...
  const summaryLines: { label: string; val: number }[] = [
            ...
            <div className="flex flex-col gap-3">
              {WINE_CATALOGUE.map(wine => (
                <WineCard
                  ...
                  onChange={(q: number) => setQty(wine.id, q)}
                />
              ))}
            </div>
            {isEmpty && (
    ...
    <div ref={ref} className="min-h-screen flex items-center justify-center px-8" style={{ background: 'var(--color-charcoal)' }}>
      <div className="flex flex-col items-start max-w-lg gap-6">
        ...
        <a data-success-item="" href="/"
          ...
          onMouseEnter={(e: React.MouseEvent<HTMLAnchorElement>) => gsap.to(e.currentTarget.querySelector('span'), { x: 5, duration: 0.35, ease: 'power2.out', overwrite: 'auto' })}
          onMouseLeave={(e: React.MouseEvent<HTMLAnchorElement>) => gsap.to(e.currentTarget.querySelector('span'), { x: 0, duration: 0.5, ease: 'elastic.out(1,0.4)', overwrite: 'auto' })}
        >
