...
/**
 * CellarGallery.tsx — Sklep u Dvořáků
 * v7 "Optimized Vault"
 * OPRAVY VÝKONU:
...
/* ── Single grid photo ───────────────────────────────────────── */
function GridPhoto({ photo }: GridPhotoProps) {
  ...
  const hovTl    = useRef<gsap.core.Timeline | null>(null);
  const bnds     = useRef<DOMRect | null>(null);
...
export default function CellarGallery() {
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      if (isDesktop) {
        cells.forEach((cell) => {
          if (!(cell as HTMLElement).dataset.parallax) return;
