 ...
 *   → umístit do /public/wine-bottle.png
 *
 * CHIRURGICKÉ ZMĚNY oproti v10:
 * – Odstraněn ScrollTrigger pin/snap — vína se nepřepínají scrollem
 * – Sekce není pinována; stránka scrolluje normálně
 * – Navigace zůstává: šipky (tlačítka + klávesnice) + swipe na dotyku
 * – Odstraněna závislost na lenis/ScrollTrigger (import zachován pro případ
 *   jiného použití na stránce, ale v této komponentě se nepoužívá)
 *
 * MOBILNÍ OPRAVY (pouze <style> blok — desktop beze změny):
 * – Vertikální stack: text nahoře, láhev dole (centrovaná, pěkně velká)
 * – Text odsazen od šipek na obou stranách (padding 58px)
 * – Nadpis se přirozeně zalomí místo ellipsis
 * – Ghost číslo skryto (při height: auto nefunguje position: absolute správně)
 */
...
const BOTTLE_SRC = '/wine-bottle.png';
...
function Corner({ pos = 'tl', accent }: CornerProps) {
  const map: Record<CornerPos, { sx: number; sy: number; top?: number; bottom?: number; left?: number; right?: number }> = {
...
export default function WineShowcase() {
  ...
  /* ── Glow morph ───────────────────────────────────────── */
  const morphGlow = useCallback((fa: string, ta: string) => {
  ...
  /* ── Master přechod ───────────────────────────────────── */
  const go = useCallback((nextIdx: number, dir = 1) => {
  ...
  useEffect(() => {
    ...
    const onKey = (e: KeyboardEvent) => {
      ...
      if (e.key === 'ArrowRight' || e.key === 'ArrowDown')
        go(((c + 1) + WINES.length) % WINES.length,  1);
    ...
    const onTM = (e: TouchEvent) => {
      ...
      if (Math.abs(dx) > Math.abs(dy) && Math.abs(dx) > 12) {
        ...
        const c    = prevRef.current;
        const dir  = dx < 0 ? 1 : -1;
        const next = ((c + dir) + WINES.length) % WINES.length;
        go(next, dir);
      }
    };
    ...
    >
      ...
      >
        ...
        <div style={{
          ...
          padding:  '0 clamp(28px,3.6vw,56px) 0 clamp(72px,6.5vw,96px)',
          position: 'relative',
          overflow: 'visible',
        }}>
        ...
        <div
          ...
          style={{
            ...
            padding:  '0 clamp(72px,6.5vw,96px) 0 clamp(28px,3.6vw,56px)',
            position: 'relative',
            overflow: 'hidden',
          }}
        >
      ...
      {(['left', 'right'] as const).map(side => {
        ...
        return (
          <button
            key={side}
            onClick={() => {
              const next = ((prevRef.current + d) + WINES.length) % WINES.length;
              go(next, d);
            }}
      ...
      <style>{`
        ...
        /* ══════════════════════════════════════════════════════
           MOBILNÍ OPTIMALIZACE  (≤ 640 px)
           ─────────────────────────────────────────────────────
           Desktop/laptop layout je beze změny.
           Cíl: text nahoře (čitelný, za šipkami), láhev dole
           (velká, centrovaná). Přirozené zalamování nadpisu.
        ══════════════════════════════════════════════════════ */
        @media (max-width: 640px) {

          /* Pravý panel schovat */
          #vina [data-right] { display: none !important; }

          /* Vertikální stack: text nahoře (1fr = zbylé místo), láhev dole (auto) */
          #vina .stage-grid {
          ...
          /* Levý panel (text): full width, doleva, padding 58px L+P
             → šipky jsou 10px od kraje + 44px wide = max 54px, text začíná na 58px ✓ */
          #vina .stage-grid > div:first-child {
            width: 100% !important;
            height: auto !important;
            justify-content: flex-start !important;
            align-items: flex-start !important;
            padding: 20px 58px 12px 58px !important;
            overflow: hidden !important;
          }

          /* Ghost číslo skrýt — position:absolute nefunguje správně při height:auto */
          #vina .stage-grid > div:first-child > div[aria-hidden] {
            display: none !important;
          }

          /* Vnitřní content div (leftRef): full width bez maxWidth omezení */
          #vina .stage-grid > div:first-child > div:not([aria-hidden]) {
            max-width: 100% !important;
            width: 100% !important;
          }

          /* Nadpis: přirozeně se zalomí, bez ellipsis, větší */
          #vina .stage-grid h2 {
          ...
          /* Popis: max 3 řádky (ušetří místo pro láhev) */
          #vina .stage-grid div:not([aria-hidden]) > p[style] {
            -webkit-line-clamp: 3 !important;
          }

          /* Láhev: full width kontejner, centrovaná, pěkně velká */
          #vina .stage-grid > div:nth-child(2) {
            width: 100% !important;
            height: clamp(200px, 37vh, 290px) !important;
          ...
          /* Vnitřní bottleRef div: omezit šířku ať láhev není příliš wide */
          #vina .stage-grid > div:nth-child(2) > div {
            width: clamp(130px, 40vw, 190px) !important;
          }
        }

        /* Nejmenší telefony (≤ 380px): ještě trochu menší nadpis, úzký strip */
        @media (max-width: 380px) {
          #vina [aria-pressed] { padding: 0 4px !important; }
          #vina .stage-grid h2 { font-size: clamp(22px, 6vw, 30px) !important; }
          #vina .stage-grid > div:first-child { padding: 16px 52px 10px 52px !important; }
        }
      `}</style>
